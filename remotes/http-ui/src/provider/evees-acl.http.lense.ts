...
import {
  ...
  PermissionType,
  UserPermissions,
} from '@uprtcl/evees-http';
import { EveesHttpUI } from './evees.http.client.remote';
...
export class EveesAccessControlHttpLense extends servicesConnect(LitElement) {
  ...
  async firstUpdated() {
    ...
    this.remote = this.evees.getRemote(this.uref) as EveesHttpUI;

    const isLoggedIn = await this.remote.isLogged();
